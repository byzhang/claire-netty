...
public:
    ...
    bool Get(const StringPiece& field, std::string* result) const
    {
        ...
        {
            if (strncasecmp(field.data(), it->first.c_str(), field.size()) == 0)
            {
                result->assign(it->second.data(), it->second.size());
                return true;
            }
        }
    ...
    bool Get(const std::string& field, std::vector<std::string>* result) const
    {
        ...
        for (auto it = headers_.cbegin(); it != headers_.cend(); ++it)
        {
            if (strncasecmp(field.data(), it->first.c_str(), field.size()) == 0)
            {
                result->push_back(std::string(it->second.data(), it->second.size()));
            }
        }
    ...
    void AppendTo(Buffer* dest) const
    {
        for (auto it = headers_.cbegin(); it != headers_.cend(); ++it)
        {
            dest->Append(it->first.c_str());
            dest->Append(": ");
            dest->Append(it->second.c_str());
            dest->Append("\r\n");
        }
        dest->Append("\r\n");
    }
